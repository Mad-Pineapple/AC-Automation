...
/** Stable identity for a designer-authored block. Slot is deliberately not
 * used as a fallback because several independent blocks can share a slot
 * (for example two decorations or two body-copy frames). */
const blockKey = (el: FreeformElement) => el.layoutBlock?.trim() || `item:${el.id}`;
  ...
  for (const [key, members] of groups) {
    ...
    for (const el of members) {
      ...
      if (el.type === "image" && members.length === 1 && stretched && !isLogo) {
        ...
        byId.set(el.id, { ...el, x: box.x, y: box.y, w: box.w, h: box.h, fit: el.keepWhole || el.shape ? "contain" : "cover" });
        continue;
      }
      ...
      if (el.type === "image" && stretched && !isLogo && (el.slot === "band" || el.slot === "photo" || el.slot === "panel")) {
        ...
        byId.set(el.id, { ...el, x: bx, y: by, w: bw, h: bh, fit: el.keepWhole || el.shape ? "contain" : "cover" });
        continue;
      }
...
export function adaptAuthoritativeBetween(
  ...
  const blocksOf = (cfg: FreeformConfig) => {
    ...
    for (const el of cfg.elements) { const k = blockKey(el); m.set(k, [...(m.get(k) ?? []), el]); }
    return m;
  };
  ...
  for (const [key, nearMembers] of bn) {
    ...
    for (const el of nearMembers) {
      ...
      if (el.type === "rect" || (el.type === "image" && !isLogo && (el.slot === "band" || el.slot === "photo" || el.slot === "panel" || nearMembers.length === 1))) {
        out.push({ ...el, x: round(box.x + fx0 * box.w), y: round(box.y + fy0 * box.h), w: Math.max(1, round((fx1 - fx0) * box.w)), h: Math.max(1, round((fy1 - fy0) * box.h)), ...(el.type === "image" ? { fit: el.keepWhole || el.shape ? "contain" : "cover" } : {}) } as FreeformElement);
        continue;
      }
