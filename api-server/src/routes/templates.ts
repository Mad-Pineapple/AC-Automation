...
async function adaptOne(
  ...
  if (masterConfig.sourceMode === "indesign-bridge" && masterConfig.authoritativeGeometry && !bridgeAntherKv) {
    ...
    if (!adapted) {
      // Build from the closest authoritative master in the target family, not
      // automatically from whichever master happened to open this request.
      // This is important when a campaign has portrait, landscape and square
      // masters: using the wrong source preserves the wrong composition even
      // when the liquid pins themselves are correct.
      const strictSource = fam.length ? selectAuthoritativeSource(fam, width, height) : null;
      const source = strictSource ?? { width: master.width, height: master.height, config: masterConfig };
      adapted = adaptAuthoritativeConfig(source.config, source.width, source.height, width, height, { rules: bridgeRules });
      method = "indesign-authoritative";
      notes.push(...(adapted.adaptNotes ?? []));
      if (source.config !== masterConfig) notes.push(`Used the closest ${source.config.masterFamily ?? "InDesign"} family master (${source.width}×${source.height}) as the authoritative source.`);
    }
  }
